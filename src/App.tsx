...
export default function App() {
  ...
  const generateAIReport = async () => {
    ...
    try {
      const prompt = `Analitza aquest perfil d'autoavaluació docent sobre l'ús de la IA.
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#5A5A40] font-medium uppercase tracking-wider">
                Marcs Educatius i Orientacions
              </p>
            </div>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        <div className="print-only mb-12 border-b-2 border-[#1A1A1A] pb-6">
          ...
          <p className="text-sm uppercase tracking-widest font-bold text-[#5A5A40]">
            Escenari: {scenario === 'teacher' ? 'Gestió Docent' : 'Activitat Alumnat'} ·{' '}
        ...
        <div className="flex flex-col md:flex-row gap-8 mb-12 no-print">
          <div className="flex-1">
            ...
            <p className="text-[#1A1A1A]/60 max-w-lg">
              L'avaluació canvia segons si fas servir la IA per a les teves tasques de gestió o si és
              per a una activitat directa amb els teus alumnes.
            </p>
          </div>
          ...
          <div className="flex bg-white p-1 rounded-2xl border border-[#1A1A1A]/10 shadow-sm self-start">
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all ${
                scenario === 'teacher' ? 'bg-[#5A5A40] text-white shadow-md' : 'hover:bg-[#F5F5F0]'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all ${
                scenario === 'student' ? 'bg-[#5A5A40] text-white shadow-md' : 'hover:bg-[#F5F5F0]'
              }`}
            >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#F5F5F0]/50 border-b border-[#1A1A1A]/10">
                  <th className="px-8 py-5 text-xs font-bold uppercase tracking-widest text-[#1A1A1A]/40">
                    Ítem d'avaluació
                  </th>
                  <th className="px-4 py-5 text-center text-xs font-bold uppercase tracking-widest text-[#1A1A1A]/40 w-24">
                    Pes
                  </th>
                  <th className="px-8 py-5 text-center text-xs font-bold uppercase tracking-widest text-[#1A1A1A]/40 w-48 no-print">
                    Validació
                  </th>
                  <th className="px-8 py-5 text-center text-xs font-bold uppercase tracking-widest text-[#1A1A1A]/40 w-24 print-only">
                    Resposta
                  </th>
                </tr>
              </thead>
                ...
                {DIMENSIONS.map((dim) => (
                  <React.Fragment key={dim}>
                    <tr className="bg-[#5A5A40]/5">
                      <td
                        colSpan={3}
                        className="px-8 py-3 text-sm font-bold text-[#5A5A40] border-y border-[#5A5A40]/10"
                      >
                        {dim}
                      </td>
                    </tr>
                    ...
                    {ITEMS.filter((item) => item.dimension === dim).map((item) => (
                      <tr
                        key={item.id}
                        className="group hover:bg-[#F5F5F0]/30 transition-colors border-b border-[#1A1A1A]/5"
                      >
                          ...
                          <div className="flex items-center justify-center gap-4">
                            <button
                              ...
                              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${
                                currentAnswers[item.id] === true
                                  ? 'bg-emerald-50 border-emerald-200 text-emerald-700 shadow-sm'
                                  : 'border-transparent text-[#1A1A1A]/30 hover:text-emerald-600'
                              }`}
                            >
                            ...
                            <button
                              ...
                              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all ${
                                currentAnswers[item.id] === false
                                  ? 'bg-rose-50 border-rose-200 text-rose-700 shadow-sm'
                                  : 'border-transparent text-[#1A1A1A]/30 hover:text-rose-600'
                              }`}
                            >
                              <XCircle size={16} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 bg-white p-8 rounded-3xl border border-[#1A1A1A]/10 shadow-lg flex flex-col items-center justify-center text-center">
            <h3 className="text-xs font-bold uppercase tracking-widest text-[#1A1A1A]/40 mb-6">
              Puntuació Global
            </h3>

            <div className="relative w-40 h-40 flex items-center justify-center mb-6">
              <svg className="w-full h-full -rotate-90">
                <circle cx="80" cy="80" r="70" fill="none" stroke="#F5F5F0" strokeWidth="12" />
                <circle
                  cx="80"
                  cy="80"
                  r="70"
                  fill="none"
                  stroke={stats.canProceed ? '#10B981' : stats.criticalFails > 0 ? '#EF4444' : '#F59E0B'}
                  strokeWidth="12"
                  strokeDasharray="440"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[10px] font-bold text-[#1A1A1A]/40 uppercase tracking-tighter">
                  Assolit
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2 w-full">
              <div className="flex justify-between text-xs font-medium">
                ...
                <span>
                  {stats.answeredCount} / {ITEMS.length}
                </span>
              </div>

              <div className="flex justify-between text-xs font-medium">
                <span className="text-[#1A1A1A]/60">Punts crítics fallats:</span>
                <span className={stats.criticalFails > 0 ? 'text-rose-600 font-bold' : 'text-emerald-600'}>
                  {stats.criticalFails}
                </span>
              </div>
            </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-serif italic mb-2">
                    {stats.canProceed
                      ? "Recurs Apte per a l'ús"
                      : stats.criticalFails > 0
                        ? 'Revisió Crítica Obligatòria'
                        : "Completa l'autoavaluació"}
                  </h3>
                ...
                {isAnalyzing ? (
                  <div className="flex flex-col items-center gap-3 text-[#1A1A1A]/40">
                    ...
                    <p className="text-xs font-medium italic">
                      L'IA està analitzant la teva reflexió pedagògica...
                    </p>
                  </div>
                ) : aiAnalysis ? (
                  ...
                  <p className="text-xs text-[#1A1A1A]/40 text-center italic">
                    Un cop omplis el formulari, prem el botó per rebre una reflexió personalitzada
                    basada en els teus resultats.
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2 opacity-40">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest">
              Guia d'ús de la IA en Educació
            </span>
          </div>
